...
export class TaskPage {
  ...
  addTask() {
    ...
    this.http.post(`http://10.12.255.27:3000/submit/task`, JSON.stringify(this.task)).subscribe((resp) => {
  ...
  editTask() {
    this.http.post(`http://10.12.255.27:3000/edit/task/${this.task.id}`, JSON.stringify(this.task)).subscribe((resp) => {
      let toast = this.toastCtrl.create({
